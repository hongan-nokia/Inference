...
dataset = datasets.load_dataset('json', data_files="jaka_robot_arm_alpaca_data.jsonl", split='train')
...
prompt_template = """
你是我的节卡机械臂助手，机械臂内置一些操作函数，请根据输入的指令，输出对应的python语句命令:
...
def generate_prompt(query, response=None, prompt_template=prompt_template):
    res = prompt_template.format(query=query)
    if response:
        res = f"{res}{response}"
    return res
...
def generate_and_tokenize_prompt(data_point):
    ...
    user_prompt = generate_prompt(query=data_point["input"])
...
trainer.train()
model.save_pretrained("lora_JAKA-32/final")
print('done')